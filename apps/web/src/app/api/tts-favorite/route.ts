...
export async function POST(request: NextRequest) {
  ...
  const { favoriteId, text } = (await request.json()) as { favoriteId: string; text: string }
  if (!favoriteId || !text?.trim()) {
    return Response.json({ error: 'favoriteId and text required' }, { status: 400 })
  }
  ...
  const storagePath = `${session.user.id}/favorites/${favoriteId}.mp3`
  ...
  // DB에 audioUrl 저장
  await fetch(`${API_URL}/favorite-expressions/${favoriteId}/audio`, {
    method: 'PATCH',
    headers: {
