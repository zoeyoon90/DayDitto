...
export async function POST(request: NextRequest) {
  ...
  if (!logId || !text?.trim()) {
    return Response.json({ error: 'logId and text required' }, { status: 400 })
  }
  ...
  // 1. Google Cloud TTS REST API 호출
  ...
  // 2. service role client로 Storage 업로드 (RLS bypass)
  const adminSupabase = createAdminClient(
    process.env.NEXT_PUBLIC_SUPABASE_URL!,
    process.env.SUPABASE_SERVICE_ROLE_KEY!,
  )

  const storagePath = `${session.user.id}/${logId}.mp3`
  ...
  // 3. NestJS PATCH /daily-logs/:id/audio — audioUrl DB 저장
  const apiUrl = process.env.NEXT_PUBLIC_API_URL ?? 'http://localhost:3001'
  await fetch(`${apiUrl}/daily-logs/${logId}/audio`, {
    method: 'PATCH',
    headers: {
