...
describe('DailyLogsService', () => {
  ...
  describe('getMonthlyLogs', () => {
      ...
      mockDb.select
        .mockReturnValueOnce({
          from: jest.fn().mockReturnValue({
            where: jest.fn().mockResolvedValue([{ timezone: 'America/New_York' }]),
          }),
        })
        .mockReturnValueOnce({
          from: jest.fn().mockReturnValue({
    ...
    it('해당 월 로그 목록 반환', async () => {
      const mockLogs = [
        ...
        { id: 'log-2', logDate: '2024-01-20', imageUrl: 'https://example.com/img.jpg' },
      ]
      mockDb.select
        .mockReturnValueOnce({
          from: jest.fn().mockReturnValue({
  ...
  describe('createLog', () => {
    ...
    const authUser = { id: 'user-1', email: 'test@test.com', provider: 'email' }
    const dto = {
      logDate: '2024-01-15',
    ...
    it('유효하지 않은 provider는 email로 처리', async () => {
      const unknownProviderUser = { id: 'user-2', email: 'x@x.com', provider: 'twitter' }
  ...
  describe('updateLineAudioUrls', () => {
    ...
    it('lineAudioUrls 업데이트 성공', async () => {
      ...
      await expect(
        service.updateLineAudioUrls('user-1', 'log-1', ['https://audio.url/line1.mp3', 'https://audio.url/line2.mp3']),
    ...
    it('빈 배열로도 업데이트 가능', async () => {
      ...
      await expect(service.updateLineAudioUrls('user-1', 'log-1', [])).resolves.not.toThrow()
    })
  })
})
