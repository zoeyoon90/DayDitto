...
describe('DailyLogsController', () => {
  ...
  describe('getMonthly', () => {
    ...
    it('month=0 → BadRequestException (1 미만)', () => {
      expect(() => controller.getMonthly(mockReq, '2024', '0')).toThrow(
        BadRequestException,
      );
    });

    it('month=13 → BadRequestException (12 초과)', () => {
      expect(() => controller.getMonthly(mockReq, '2024', '13')).toThrow(
        BadRequestException,
      );
    });

    it('month=-1 → BadRequestException', () => {
      expect(() => controller.getMonthly(mockReq, '2024', '-1')).toThrow(
        BadRequestException,
      );
    });

    it('year 빈 문자열 → BadRequestException (parseInt 결과 NaN)', () => {
      expect(() => controller.getMonthly(mockReq, '', '6')).toThrow(
        BadRequestException,
      );
    });

    it('month 빈 문자열 → BadRequestException', () => {
      expect(() => controller.getMonthly(mockReq, '2024', '')).toThrow(
        BadRequestException,
      );
    });

    it('year에 숫자 아닌 문자열 → BadRequestException', () => {
      expect(() => controller.getMonthly(mockReq, 'abc', '6')).toThrow(
        BadRequestException,
      );
    });
    ...
    it('year=0 → BadRequestException (!year는 0도 falsy)', () => {
      ...
      expect(() => controller.getMonthly(mockReq, '0', '6')).toThrow(
        BadRequestException,
      );
    });
  ...
  describe('updateAudio', () => {
    it('audioUrl 업데이트 서비스 호출', () => {
      ...
      controller.updateAudio(mockReq, 'log-1', {
        audioUrl: 'https://audio.url/1.mp3',
      });

      expect(service.updateAudioUrl).toHaveBeenCalledWith(
        'user-1',
        'log-1',
        'https://audio.url/1.mp3',
      );
    });
  });
  ...
  describe('updateLineAudio', () => {
    it('lineAudioUrls 업데이트 서비스 호출', () => {
      ...
      controller.updateLineAudio(mockReq, 'log-1', {
        lineAudioUrls: ['url1', 'url2'],
      });

      expect(service.updateLineAudioUrls).toHaveBeenCalledWith(
        'user-1',
        'log-1',
        ['url1', 'url2'],
      );
